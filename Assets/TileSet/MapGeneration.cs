...
public class MapGeneration : MonoBehaviour
{
    ...
    Dictionary<Vector4Int, List<TileObject>> Mapping;
    ...
    void InitMapping()
    {
        Mapping = new Dictionary<Vector4Int, List<TileObject>>();
        ...
        foreach (var tile in Tiles)
        {
            for (int i = 0; i < 4; i++)
            {
                ...
                Vector4Int v = tile.Object.GetComponent<Tile>().GetRect(rot);
                // if (Mapping.ContainsKey(v)) continue;

                TileObject to = new TileObject();

                to.Weight = tile.Weight;
                to.Object = Instantiate(tile.Object, Template);
                to.Object.GetComponent<Tile>().tile_rotation = rot;
                to.Object.transform.position = new Vector2(1000, 1000); // off screen
                if (!Mapping.ContainsKey(v)) Mapping.Add(v, new List<TileObject>());
    ...
    {
        const float WaterProportion = 0.1f;
        GridRect = new Vector4Int[GridSize.x, GridSize.y];
        GridObject = new GameObject[GridSize.x, GridSize.y];
        ...
        for (var x = 0; x < GridSize.x; ++x)
        {
            for (var y = 0; y < GridSize.y; ++y)
            {
                GridRect[x, y] = new Vector4Int(Random.value > WaterProportion, Random.value > WaterProportion, Random.value > WaterProportion, Random.value > WaterProportion);
                GridObject[x, y] = CreateObject(GridRect[x, y], new Vector2Int(x, y));
            }
        }
    }
    ...
    GameObject RandomTile(Vector4Int r)
    {
        ...
        {
            rand -= t.Weight;
            if (rand <= 0) return t.Object;
        }
        return Mapping[r][0].Object;
    }
    ...
    void Update()
    {
        ...
        var hits = Physics.RaycastAll(ray, float.PositiveInfinity, Mask);
        if (hits.Length != 0)
        {
